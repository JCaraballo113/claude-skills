...
Features develop test-first (red → green → refactor) via the `tdd` skill. The generated rule tells the agent that when the skill fails the installed-check (wording in SKILL.md), it prompts the user to install it (command in [skill.deps.json](../skill.deps.json)) rather than improvising the loop.
