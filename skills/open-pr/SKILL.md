...
- Do not batch questions. One at a time, with a recommendation each.
- Do not skip a question to be polite. Skip only when the answer is unambiguous from the diff.
...
- Do not invent a Conventional Commits scope to fill the parens. Drop them.
- Do not put ticket IDs in the title. They go in the footer.
