...
| Skill                                                                | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| -------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [agent-rules](./skills/agent-rules/SKILL.md)                         | Encode working conventions as one rule file per concern in `.claude/rules/` — testing (TDD, happy/negative path, coverage as discovery), migrations, design-system (atomic vocabulary + content extremes), code-review, sub-agents (model tiering), finding-unknowns — generalized to the project, any ecosystem.                                                                                                                                                                                                                                                   |
| [autopilot](./skills/autopilot/SKILL.md)                             | Orchestrate a batch of tickets or fixes to done, hands-off: the main session orchestrates (never implements) — each item runs in its own git worktree where a fresh agent (briefed via `/autopilot-handoff`, driven by `/autopilot-implement`) implements, commits by pathspec, and attaches a `git note` with its provenance + reviewer notes; the orchestrator validates and fast-forwards each into the branch, holds front-end work to a design-parity gate against every variant the design defines, and reports the run as an `/html-artifact`. User-invoked. |
| [autopilot-handoff](./skills/autopilot-handoff/SKILL.md)             | Model-invocable form of `/handoff`, used by [`autopilot`](./skills/autopilot/SKILL.md) to brief each per-item agent — a pickup doc with a suggested-skills section, artifacts referenced by path, secrets redacted. Scoped so it fires only inside an autopilot run.                                                                                                                                                                                                                                                                                                |
| [autopilot-implement](./skills/autopilot-implement/SKILL.md)         | Model-invocable form of `/implement`, run by [`autopilot`](./skills/autopilot/SKILL.md)'s per-item agents in their worktree — implement, TDD at the seams, typecheck + tests, `/code-review`, then commit by pathspec and attach a `git note`, returning a terse result the orchestrator validates and fast-forwards in. Scoped to autopilot.                                                                                                                                                                                                                       |
