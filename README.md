...
| Skill | Description |
|---|---|
| [agent-rules](./skills/agent-rules/SKILL.md) | Encode working conventions as one rule file per concern in `.claude/rules/` — testing (TDD, happy/negative path, coverage as discovery), migrations, design-system (atomic vocabulary + content extremes), code-review, sub-agents (model tiering), finding-unknowns, agent-summaries (debrief, not changelog) — generalized to the project, any ecosystem. |
